...
def resize_and_crop(image_array, size, eye_dis, landmarks):
    left_eye = landmarks['left_eye']
    right_eye = landmarks['right_eye']

    left_eye_center = np.mean(left_eye, axis=0).astype("int")
    right_eye_center = np.mean(right_eye, axis=0).astype("int")
    raw_eye_distance =np.sqrt((left_eye_center[0] - right_eye_center[0]) ** 2 + (left_eye_center[1] - right_eye_center[1]) ** 2)

    ratio = eye_dis / raw_eye_distance
    h, w, c = image_array.shape

    pro_h, pro_w = int(h*ratio), int(w*ratio)
    image_array = cv2.resize(image_array, (pro_w, pro_h), interpolation=cv2.INTER_CUBIC)
    transferred_landmarks = defaultdict(list)
    for facial_feature in landmarks.keys():
        for landmark in landmarks[facial_feature]:
            transferred_landmark = (int(landmark[0]*ratio), int(landmark[1]*ratio))
            transferred_landmarks[facial_feature].append(transferred_landmark)
    img = np.zeros((size[0], size[1], 3))  # 0 250  1 200
    left_eye = transferred_landmarks['left_eye']
    left_eye_center = np.mean(left_eye, axis=0).astype("int")
    h, w, c = image_array.shape
    padding_x = padding_y = 0
    left_upper_x = left_eye_center[0]-75
    if left_upper_x < 0:
        padding_x = abs(left_upper_x)
        left_upper_x = 0
    left_upper_y = left_eye_center[1] - 125
    if left_upper_y < 0:
        padding_y = abs(left_upper_y)
        left_upper_y = 0
    right_low_x = left_eye_center[0] + 125
    right_low_x = right_low_x if right_low_x < w else w
    right_low_y = left_eye_center[1] + 125
    right_low_y = right_low_y if right_low_y < h else h
    img[padding_y:padding_y+right_low_y-left_upper_y, padding_x:padding_x+right_low_x-left_upper_x, :] = image_array[left_upper_y:right_low_y, left_upper_x:right_low_x, :]
    cv2.imwrite('/data/xxx/444_save12.jpg', img)
    return img
...
def face_preprocess(image, landmark_model_type='large', crop_size=[250, 200], eye_dis=50):
    ...
    cropped_face = resize_and_crop(image_array=aligned_face, size=crop_size, eye_dis=eye_dis, landmarks=rotated_landmarks)
    # transfer landmarks to fit the cropped face
...
def save_landmark(image_array, landmarks, img_name):
    ...
    for facial_feature in landmarks.keys():
        for point in landmarks[facial_feature]:
            cv2.circle(image_array, (int(point[0]), int(point[1])), 1, (0, 0, 255), 4)
    cv2.imwrite(img_name, image_array)


if __name__ == '__main__':
    # load image
    img_name = '/data/xxx/444.jpg'
    img_save = '/data/xxx/444_save.jpg'
    image_array = cv2.imread(img_name)

    # preprocess the face image
    face = face_preprocess(image=image_array, landmark_model_type='large',)

    # visualize_landmark(image_array=face, landmarks=landmarks)

    cv2.imwrite(img_save, face)
